<?php 
	...
	$sql="SELECT * FROM willdev_quotes
			LEFT JOIN willdev_category
			ON willdev_quotes.`cat_id`=willdev_category.`cid` 
			WHERE willdev_quotes.`status`='1' AND willdev_quotes.`id`='$id'";

	$res=mysqli_query($mysqli,$sql);
	$row=mysqli_fetch_assoc($res);


 	$sql1="SELECT willdev_reports.*, willdev_users.`name`, willdev_users.`user_image` FROM willdev_reports, willdev_users WHERE willdev_reports.`post_id`='$id' AND willdev_reports.`report_type`='quote' AND willdev_users.`id`=willdev_reports.`user_id` ORDER BY willdev_reports.`report_on` DESC";
